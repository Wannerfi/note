...
## lua protobuf 编解码错误
**问题描述**
解码：pd协议之前的字段只有两个string，还没有产生问题。后面添加一个uint32字段后，网络接包并解码后，新增的uint32字段只有前一个字节被成功解析。测试过 pb 本身没有问题
编码：pb 编码后，发送到服务器后，服务器解码后数据错误。

**产生原因**
解码：在c++端接收网络包，此时包为pd编码后的数据，转到lua上需要将网络包转为 FString （宽字符），在转换的过程中出现问题，导致在 lua 上用pb解码错误。
编码：在lua端进行pb编码后，传到c++端后pb数据变成了宽字符，从而产生了错误。

**解决**
重点是 pb 数据在c++端和lua端的传输要保持原来的字符串，这里用const char* 解决（bug: 可能pb数据有提前终止符？）
解码问题用 CustomDelegate，因为在c++中收到消息后，是通过广播传递到lua中去的，广播的参数固定，所以使用自定义广播的数据类型 const char*。
编码问题在于lua 到 c++ 的交互，与解码问题有相似之处，但是解码问题涉及到广播，所以要用自定义的委托。而利用反射的交互方式就不得不使用FString，即宽字符，所以更改交互方式，使用自定义lua的类和方法，将FNetworkMgr类包成单例，从游戏实例持有改成静态变量，这样就可以用const char* 与lua交互了。自定义方法如下：
```cpp
DefLuaClass(FNetworkMgr)
DefLuaMethod(Start, &FNetworkMgr::Start)
//...
EndDef(FNetworkMgr, &FNetworkMgr::Get)
```
